...
int main(int argc, char *argv[]){
    ...
    */
    ...
    // f - File pointer, words - number of words, c - character in file.
    FILE *f;
    int words, inWord = 0;
    char c;

    // Open the file in read mode.
    f = fopen("Text_File_To_Be_Transferred.txt", "r");

    // Counting number of words
    while((c = fgetc(f)) != EOF){
        if (c == ' ' || c == '\t' || c == '\n') {
            if (inWord) {
                inWord = 0;  // Exit the current word
                words++;
            }
        } else {
            if (!inWord) {
                inWord = 1;  // Enter a new word
            }
        }
    }

    // Send the word count
    n = send(sockfd, (char *)&words, sizeof(words), 0);

    // Error while sending the file
    if(n < 0){
        error("Error sending the number of words.");
    }

    // File pointer is at EOF. Making it point to the BOF again.
    rewind(f);

    // Sending the file to the server word by word.
    while(fscanf(f, "%s", buffer) != EOF){
        // Send the file
        n = send(sockfd, (char *)&buffer, 255, 0);
        // Error while sending the file
        if(n < 0){
    ...
    // File Transferred
    printf("The file has been successfully transferred. Thank you.");
